...
export function useCurrencies() {
    ...
    const syncCurrency = useCallback(async (currencyType: Adhoc.CurrencyType) => {
        try {
            ...
            if (res.status !== 'error') {
                ...
                // TODO GlacierLuo 不确定的fix
                if (
                    currencyType === 'sanity' &&
                    Array.isArray(increase_policy) &&
                    increase_policy.length === 1 &&
                    increase_policy[0][0] === 0 &&
                    increase_policy[0][1] === 0
                ) {
    ...
    // 更新单个货币余额（基于本地数据重新计算） TODO Glacierluo 后续考虑如何处理
    const updateCurrency = useCallback(
        (currencyType: Adhoc.CurrencyType) => {
            setCurrencyData((prev) => {
                ...
                if (data) {
                    ...
                    return {
                        ...
                        [currencyType]: {
                            ...data,
                            balance: calcCurrentBalance(data.originBalance, data.increasePolicy),
                        },
                    };
                }
                return prev;
            });
            syncCurrency(currencyType);
        },
        [syncCurrency],
    );
