...
export function SettingPage() {
    const { usePuzzleList, setUsePuzzleList, kelanOverride, setKelanOverride } = useContext(SiteSettingContext);
    return (
        <div className={'slim-container'}>
            ...
            <FancyCard title="常规设置">
                ...
                <br />
                <InputWithLabel label={'柯蓝状态'} extra={'手动选择柯蓝的显示状态，或跟随理智值自动切换。'}>
                    <div className="join">
                        {(['auto', '1', '2', '3', '4'] as const).map((value) => (
                            <input
                                ...
                                type="radio"
                                name="kelan"
                                aria-label={value === 'auto' ? '自动' : `状态${value}`}
                                checked={kelanOverride === value}
                                onChange={(e) => {
                                    if (e.target.checked) setKelanOverride(value);
                                }}
                            />
                        ))}
                    </div>
