...
function ManualHintContent({
    ...
    if (cooling) {
        requestManualComponent = <DisabledInfo disabledReason={`本题外援解锁倒计时：${format_ts_to_HMS(countdown)}`} />;
    } else if (ticketHintInfo.disabled) {
        let activeHintsReason = '';
        if (ticketHintInfo.hints_open && ticketHintInfo.hints_open.length > 0) {
            activeHintsReason =
                '当前 ' + ticketHintInfo.hints_open.map((s) => `【${s}】`) + '题目的外援处于未完成状态。';
        }
        requestManualComponent = (
            <DisabledInfo disabledReason={(ticketHintInfo.disabled_reason ?? '请求外援已禁用！') + activeHintsReason} />
        );
    } else
        requestManualComponent = (
...
export function ManualHintTab({ puzzleData }: { puzzleData: Wish.Puzzle.PuzzleDetailData }) {
    ...
    const bannedText = `您的队伍的外援功能被禁用至 ${format_ts(info.team!.ban_list.ban_manual_hint_until)}。`;
    ...
    return (
        <div>
            ...
            {data.data.list.length > 0 && (
                <div className="mt-5">
                    <h2 className="text-3xl font-medium">外援列表</h2>
