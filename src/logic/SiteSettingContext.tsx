...
import { KelanOverride, PuzzleListConfig, SiteSettingContext, SiteSettingContextType, useSuccessGameInfo } from '@/logic/contexts.ts';
...
export function GameSettingContextProvider({ children }: { children: ReactNode }) {
    ...
    const [kelanOverride, setKelanOverride] = useState<KelanOverride>('auto');

    const setDefaultContext = (key: string) => {
        localStorage.setItem(key, encodeObjectToBase64({ usePuzzleList: 'off', kelanOverride: 'auto' }));
    };
    ...
    useEffect(() => {
        ...
        //@ts-ignore
        if (obj.kelanOverride) setKelanOverride(obj.kelanOverride);
    }, [key]);
    ...
    return (
        <SiteSettingContext.Provider
            value={{
                ...
                kelanOverride,
                setKelanOverride: (value: KelanOverride) => {
                    localStorage.setItem(
                        key,
                        encodeObjectToBase64({
                            usePuzzleList,
